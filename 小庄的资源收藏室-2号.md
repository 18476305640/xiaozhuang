    ...
    <div class="modal-content">
      ...
      <div class="manual-input-section" style="display: none;">
        <div class="form-group">
          ...
          <textarea class="config-textarea" id="manualConfigInput" placeholder='示例：{"token":"ghp_xxx","owner":"用户名","repo":"仓库名","branch":"main"}'></textarea>
        </div>
...
-- view:js --
    ...
    class GitHubEditor {
      ...
      constructor() {
        ...
        waitForClassicEditor(() => this.#initCKEditor())
      }
      ...
      /**
       * 打开文件（新增：切换文件时保存本地缓存 + 清空缓存状态）
       */
      async openFile(path, name, fileElement) {
        // 1. 保存当前内容（可能是本地缓存或未保存的文件更改）
      ...
      /**
       * 统一内容保存入口（核心优化：自动判断保存到GitHub还是localStorage）
       */
      async #saveContent() {
        // 1. 已选择文件：保存到GitHub
      ...
      /**
       * 保存到localStorage（新增：本地缓存核心方法）
       */
      #saveToLocalStorage() {
        const content = this.#editor.getData();
        localStorage.setItem(this.#LOCAL_STORAGE_KEY, content);
        this.#isContentChanged = false;
        this.updateStatus('本地缓存已更新', 'success');
      }

      /**
       * 从localStorage加载内容（新增：初始化/切换文件时调用）
       */
      #loadLocalCache() {
        if (this.#currentFilePath) return; // 已选择文件时不加载缓存
        
        const cachedContent = localStorage.getItem(this.#LOCAL_STORAGE_KEY) || '';
