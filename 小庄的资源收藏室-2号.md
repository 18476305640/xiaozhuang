...
## 使用场景与配置推荐
| 用途          | CPU（频率/核心）                   | 内存（容量/频率）            | 显卡                                            | 硬盘（速度）                                  | 主板（Z/B/H系列）         | 显示器（分辨率/刷新率）            | 推荐品牌（国内常见）                                                                                                                      | 注意点 & 为什么                                                                                                            |
| ----------- | ---------------------------- | -------------------- | --------------------------------------------- | --------------------------------------- | ------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **办公**      | 中低端，主频 ≥3.0GHz，4-6 核即可       | 16GB，3200MHz         | 核显/入门独显                                       | SSD 500MB/s 以上（SATA 就够）                 | B 或 H 系列即可          | 1080p，60Hz              | CPU: Intel i5 / AMD R5<br>内存: 金士顿(Kingston)、威刚(ADATA)<br>硬盘: 西部数据(WD)、三星(Samsung)、铠侠(Kioxia)<br>主板: 华硕(ASUS)、微星(MSI)、华擎(ASRock) | **原因**：办公主要是文档、表格、网页 → 对显卡要求低，核显够用；16GB 内存保证不卡顿；SSD（即使是 SATA）也比机械硬盘快很多。显示器 1080p/60Hz 足够清晰。                          |
| **游戏**      | 中高端，主频 ≥3.5GHz，6-8 核以上       | 16-32GB，3600-4000MHz | 中高端独显（NVIDIA RTX 4060Ti\~4080 / AMD RX 7700+） | NVMe SSD ≥3000MB/s                      | B 或 Z 系列（支持超频/高频内存） | 2K/4K，144Hz 以上          | 显卡: NVIDIA/华硕/七彩虹(Colorful)、微星<br>内存: 金百达(KingBank)、芝奇(G.Skill)<br>硬盘: 三星 980 Pro、致钛(Zhitai)<br>显示器: AOC、HKC、戴尔                 | **原因**：游戏吃显卡，CPU 要求次之 → 显卡是最关键；高频内存能提升帧数；NVMe SSD 让读盘快、进游戏快；144Hz 带来流畅体验。显示器要根据游戏类型选：FPS 游戏建议 2K/144Hz，画质党可选 4K/高刷新。 |
| **设计/渲染**   | 高端，多核 ≥8-16 核，频率 ≥3.5GHz     | 32-64GB，3600MHz 以上   | 高性能独显（RTX 4070Ti\~4090，专业可选 NVIDIA Quadro）    | NVMe SSD ≥3500MB/s + 大容量 HDD            | Z 系列（支持更多扩展）        | 4K，60-120Hz，色域好（专业级显示器） | CPU: Intel i9 / AMD R9<br>显卡: NVIDIA RTX A 系列、RTX 4090<br>内存: 芝奇、海盗船(Corsair)<br>硬盘: 三星、铠侠、致钛<br>显示器: 明基(BenQ)、戴尔(Dell) 专业屏     | **原因**：渲染吃 CPU 核心和显卡算力 → CPU 多核、显卡高算力必选；内存大保证项目不卡；硬盘既要快（NVMe）又要大（HDD 存素材）；显示器要精准色彩，支持广色域/校色。                         |
| **服务器/工作站** | 至强/EPYC，16-32 核以上，频率 ≥2.5GHz | 64GB+ ECC 内存，3200MHz | 专业显卡或无需显卡                                     | 企业级 NVMe SSD + RAID 阵列（速度 3000MB/s+，容错） | C 系列或工作站/服务器专用主板    | 可选带远程管理，显示器不重要          | CPU: Intel Xeon / AMD EPYC<br>内存: 三星、金士顿 ECC<br>硬盘: 西数企业级、希捷 Exos<br>主板: 超微(Supermicro)、华硕工作站<br>电源: 海韵(Seasonic)、长城            | **原因**：工作站/服务器强调 **稳定性和数据安全**，所以要选 ECC 内存（防止内存错误）、企业级 SSD/HDD（寿命长），并配置 RAID（数据冗余）。CPU 要高并发处理能力，显示器无所谓。               |
