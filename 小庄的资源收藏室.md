...
# [程序员]WSL2+Ubuntu安装Docker（在windows上安装ubuntu在Ubuntu上安装docker部署开发环境）
...
wsl --export Ubuntu D:/ubuntu-swl-export.tar
#导入实例（这里第一个位置参数才是迁移后的位置）
wsl --import Ubuntu D:/wsl/ubuntu D:/ubuntu-swl-export.tar --version 2
#这里就迁移到了D:/wsl/ubuntu这个位置了
```

5、在swl的ubuntu上安装docker
按“徽键”点击小鸟图标的“swl”打开默认的swl（`wslconfig /l`查看默认的swl实例）
进入实例后,执行以下命令安装：
```bash
