...
const getInitConversationHistory = (()=>{
  ...
  let lastPrompt = "";
  function historyToView(history = []) {
    debugger
    // 清理掉所有历史
    historyContainer.innerHTML = '';
    // 重构
    for(let item of history) appendToConversationHistory(item.role, item.content);
    // 到底部
    const page = document.querySelector("#text_show");
    page.scrollTop = page.scrollHeight;
  }
  function checkLastAsk(history = [], userInput) {
    debugger
    // 从右往左遍历
    for (let i = history.length - 1; i >= 0; i--) {
        if (history[i].rule === 'user') {
            if (history[i].content !== userInput) {
                return history; // 最近的用户提问不匹配，直接返回原 history
            } else {
                // 从历史记录中移除
                const newHistory = history.slice(0, i);
                historyToView(newHistory);
                return newHistory; // 移除该 rule 右边的所有项
            }
        }
    }
  ...
  return async function(userInput) {
    ...
    if("[WS]" === prompt) {
      ...
      // html精简
      bodyHtml = cleanHtml(bodyHtml, ['style','script'], ['class','id','href','title','alt']);
    ...
    // 在这里看是否重复问了
    conversationHistory = checkLastAsk(conversationHistory,userInput);
